...
resource "aws_instance" "jenkins_primary" {
    ...
    vpc_security_groups_ids = [aws_secuirty_group.jenkins_sg_primary.id]
...
resource "aws_instance" "jenkins_secondary" {
    ...
    associate_public_ip_address = true
    vpc_security_groups_ids = [aws_secuirty_group.jenkins_sg_secondary.id]
    subnet_id = aws_subnet.secondary_subnet_1.id

    tags = {
        Name = join("_", ["jenkins_primary_tf", count.index + 1])
    }
