...
resource "aws_instance" "jenkins-secondary" {
  ...
  #   when = destroy
  #   inline = [
  #     "java -jar /home/ec2-user/jenkins-cli.jar -auth @/home/ec2-user/jenkins_auth -s http://${aws_instance.jenkins-primary.private_ip}:80 delete-node ${self.private_ip}"
  #   ]
  #   connection {
  #     type        = "ssh"
  #     user        = "ec2-user"
  ...
  provisioner "local-exec" {
    command = "aws ec2 wait instance-status-ok --region ${var.region-secondary} --instance-ids ${self.id}\nansible-playbook --extra-vars 'passed_in_hosts=tag_Name_${self.tags.Name} primary_ip=${aws_instance.jenkins-primary.private_ip}' ansible_templates/install_secondary.yaml"
  }
  tags = {
...
#     when = destroy
#     inline = [
#       "java -jar /home/ec2-user/jenkins-cli.jar -auth @/home/ec2-user/jenkins_auth -s http://${aws_instance.jenkins-primary.private_ip}:8080 -auth @/home/ec2-user/jenkins_auth delete-node ${self.private_ip}"
#     ]
#     connection {
#       type        = "ssh"
#       user        = "ec2-user"
